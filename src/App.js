...
function App() {
  ...
  const [chat, setChat] = useState([{ role: "system", content: "You are a helpful assistant. You get prompts which get generated by speech input through a microphone. You have a token limit of 100, so make sure to only generate full sentences such that they do not get cut off." }]);
  ...
  async function sendChatGptRequest(chat) {
    ...
    //Cutting everything off until the last dot if there is an unfinished sentence
    const lastDotIndex = response.lastIndexOf('.');
    let cutOffText = '';
    if (lastDotIndex !== -1) {
      cutOffText = response.substring(lastDotIndex + 1);
      response = response.substring(0, lastDotIndex + 1);
    }

    console.log("This is the response: " + response);
    console.log("This is the cut off string : " + cutOffText);
